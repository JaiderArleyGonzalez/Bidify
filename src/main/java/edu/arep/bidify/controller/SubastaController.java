package edu.arep.bidify.controller;

import edu.arep.bidify.dto.MessageDto;
import edu.arep.bidify.model.Subasta;
import edu.arep.bidify.model.Usuario;
import edu.arep.bidify.service.SubastaService;
import edu.arep.bidify.service.UsuarioService;
...
public class SubastaController{
    ...
    @GetMapping("/{subastaId}/{numBids}/time")
    public String testScenario (@PathVariable int subastaId, @PathVariable int numBids){
        Subasta subasta = subastaService.getSubastaById(subastaId).orElse(null);
        String message = "";
        if(subasta != null && subasta.isEstado()){
            long startTime = System.nanoTime();
            for(int i = 0; i < numBids; i++){
                subastaService.recibirPuja(usuarioService.getUsuarioById(3), (subasta.getPrecioFinal().add(new BigDecimal(i))));
            }
            long endTime = System.nanoTime();
            long elapsedTime = endTime - startTime;
            message = "Tiempo transcurrido en nanosegundos: " + elapsedTime;
        }
        return message;
    }
